...
Eigen::MatrixXd sparse_gauss_newton(IntrinsicGeometryInterface& geometry,
                                    ...
                                    const std::function<void(const Eigen::VectorXd&)>& callback)
{
  ...
  std::cout << "Initial SPN energy: " << energy << " " << energy << std::endl;
  ...
  for(int i = 0; i < max_iters; ++i)
  {
    ...
    deltaX = P.transpose() * deltaX;
        ...
        std::cout << "deltaX:" << deltaX.cwiseAbs().maxCoeff() << '\n';
    std::cout << "deltaTheta:" << deltaTheta.cwiseAbs().maxCoeff() << '\n';
