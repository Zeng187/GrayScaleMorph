...
int main(int argc, char *argv[])
{
    ...
    auto V_para = V;
    V_para.col(0) = P.col(0);
    V_para.col(1) = P.col(1);
    V_para.col(2).fill(0);
    V_para *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_para" + ".obj",
                  V_para, F);
    ...
    spdlog::info("Step 2: Boundary condition Settings.");
    ...
    spdlog::info("Step 3: Material Settings.");
    ...
    Morphmesh::ComputeTLayersFromMorphophing(morph_mesh.lambda_pf_s, morph_mesh.kappa_pf_s,
                                             ac.m_strain_curve, ac.m_moduls_curve, ac.thickness,
                                             morph_mesh.t_layer_pf_1, morph_mesh.t_layer_pf_2);

    // Morphmesh::RestrictRange(morph_mesh.t_layer_pv_1, 0.0, 1.0);
    // Morphmesh::RestrictRange(morph_mesh.t_layer_pv_2, 0.0, 1.0);
    // Morphmesh::RestrictRange(morph_mesh.t_layer_pf_1, 0.0, 1.0);
    // Morphmesh::RestrictRange(morph_mesh.t_layer_pf_2, 0.0, 1.0);
    ...
    Morphmesh::ComputeMorphingFormTLayers(morph_mesh.t_layer_pf_1, morph_mesh.t_layer_pf_2,
                                          ac.m_strain_curve, ac.m_moduls_curve, ac.thickness,
    ...
    FaceData<double> t_layer_pf_1_s(mesh, morph_mesh.t_layer_pf_1);
    FaceData<double> t_layer_pf_2_s(mesh, morph_mesh.t_layer_pf_2);
    ...
    spdlog::info("Step 4: Forward Predit.");

    auto Vr = V;
    auto Vout = V;

    // (1) Simulation from material curve paramters t_layer_pv
    auto simul_func_t_pv = simulationFunctionWithMaterial(geometry,
                                                          MrInv,
                                                          t_layer_pv_1_s,
                                                          t_layer_pv_2_s,
                                                          ac.m_strain_curve,
    ...
    // Vr.col(1) = P.col(1);
    // Vr.col(2).fill(0);
    newton(geometry, Vr, simul_func_t_pv, config.RuntimeSetting.MaxIter,
           config.RuntimeSetting.epsilon, true, fixedIdx);

    Vout = Vr;
    Vout *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_pred_t_pv" + ".obj",
                  Vout, F);

    // (2) Simulation from material curve paramters t_layer_pf
    auto simul_func_t_pf = simulationFunctionWithMaterial(geometry,
                                                          MrInv,
                                                          t_layer_pf_1_s,
                                                          t_layer_pf_2_s,
                                                          ac.m_strain_curve,
    ...
    // Vr.col(1) = P.col(1);
    // Vr.col(2).fill(0);
    newton(geometry, Vr, simul_func_t_pf, config.RuntimeSetting.MaxIter,
           config.RuntimeSetting.epsilon, true, fixedIdx);

    Vout = Vr;
    Vout *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_pred_t_pf" + ".obj",
                  Vout, F);

    // (3) Simulation from lambda and kappa per vertex
    auto simul_func_lk_pv = simulationFunction(geometry,
                                               MrInv,
                                               lambda_pv_s,
                                               kappa_pv_s,
                                               E, nu, ac.thickness, config.RuntimeSetting.w_s, config.RuntimeSetting.w_b);
    ...
    Vr = V;
    newton(geometry, Vr, simul_func_lk_pv, config.RuntimeSetting.MaxIter,
           config.RuntimeSetting.epsilon, true, fixedIdx);

    Vout = Vr;
    Vout *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_pred_lk_pv" + ".obj",
                  Vout, F);

    // (4) Simulation from lambda and kappa per face
    auto simul_func_lk_pf = simulationFunction(geometry,
                                               MrInv,
                                               lambda_pf_s,
                                               kappa_pf_s,
                                               E, nu, ac.thickness, config.RuntimeSetting.w_s, config.RuntimeSetting.w_b);
    ...
    Vr = V;
    newton(geometry, Vr, simul_func_lk_pf, config.RuntimeSetting.MaxIter,
           config.RuntimeSetting.epsilon, true, fixedIdx);

    Vout = Vr;
    Vout *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_pred_lk_pf" + ".obj",
                  Vout, F);

    // (5) Simulation from lambda and kappa per face
    auto simul_func_lk_pfv = simulationFunction(geometry,
                                                MrInv,
                                                lambda_pf_s,
                                                kappa_pv_s,
                                                E, nu, ac.thickness, config.RuntimeSetting.w_s, config.RuntimeSetting.w_b);
    ...
    Vr = V;
    newton(geometry, Vr, simul_func_lk_pfv, config.RuntimeSetting.MaxIter,
           config.RuntimeSetting.epsilon, true, fixedIdx);

    Vout = Vr;
    Vout *= 1.0 / scaleFactor;
    igl::writeOBJ(config.OutputSetting.OutputPath +
                      config.ModelSetting.ModelName + "_pred_lk_pfv" + ".obj",
                  Vout, F);
