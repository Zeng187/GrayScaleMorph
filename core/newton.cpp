                                    ...
                                    const std::function<void(const Eigen::VectorXd&)>& callback)
{
  ...
  // Face-space regularization for theta2 (kappa, |F|)
  const int nF_reg = static_cast<int>(mesh.nFaces());

  Eigen::SparseMatrix<double> M_theta(nF_reg, nF_reg);
  M_theta.reserve(nF_reg);
  {
    int iF = 0;
    for(Face f : mesh.faces()) {
  ...
  Eigen::SparseMatrix<double> L(nF_reg, nF_reg);
  {
    ...
    std::vector<double> diag(nF_reg, 0.0);
    ...
    for(int i = 0; i < nF_reg; ++i) trips.emplace_back(i, i, diag[i]);
  ...
  auto distance = [&](const Eigen::VectorXd& th) {
    ...
    auto simFunc = simulationFunction(geometry, MrInv, theta1, theta2, E_face, nu, h, w_s, w_b, ref_faces);
                                  ...
                                  const std::function<void(const Eigen::VectorXd&)>& callback)
{
  ...
  auto distance = [&](const Eigen::VectorXd& th) {
    ...
    // IMPORTANT: call the overload with (FaceData<double> lambda, FaceData<double> kappa)
    auto simFunc = simulationFunction(geometry, MrInv, theta1, theta2, E_face, nu, h, w_s, w_b, ref_faces);
                                    ...
                                    const std::function<void(const Eigen::VectorXd&)>& callback)
{
  ...
  // Face-space regularization for theta1 (lambda, |F|)
  const int nF_reg = static_cast<int>(mesh.nFaces());
