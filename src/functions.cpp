...
// kappa is per-face (|F| DOFs), no vertex-to-face averaging
TinyAD::ScalarFunction<1, double, Eigen::Index> adjointFunction_FixLam_OptKapPF(IntrinsicGeometryInterface &geometry,
                                                                              const Eigen::MatrixXi &F,
                                                                              const FaceData<Eigen::Matrix2d> &MrInv,
                                                                              const FaceData<double> &lambda,
                                                                              double E,
                                                                              ...
                                                                              double w_b)
{
  ...
  // Variables: [x(3|V|), kap(|F|)]
  TinyAD::ScalarFunction<1, double, Eigen::Index> func =
      TinyAD::scalar_function<1>(TinyAD::range(3 * mesh.nVertices() + mesh.nFaces()));

  // 1st fundamental form (stretching) — same as existing, kappa not involved
  func.add_elements<9>(TinyAD::range(F.rows()),
                       [&, alpha, beta, E, nu, h, w_s, w_b, lambda](auto &element) -> TINYAD_SCALAR_TYPE(element)
                       {
                         using T = TINYAD_SCALAR_TYPE(element);
  ...
  // 2nd fundamental form (bending) — kappa is per-face, ONE variable per face
  geometry.requireVertexIndices();
  ...
  func.add_elements<3 * 6 + 1>(
      ...
      [&, alpha, beta, E, nu, h, w_s, w_b, lambda](auto &element) -> TINYAD_SCALAR_TYPE(element)
      {
        ...
        // Per-face kappa: directly access face variable (NO averaging!)
        T kap = element.variables(3 * mesh.nVertices() + f_idx)(0);
...
// Per-face kappa version of FixKap_OptLam2
// Variables: [x (3|V|), lambda (|F|)], kappa is FaceData constant
TinyAD::ScalarFunction<1, double, Eigen::Index>
adjointFunction_FixKap_OptLam2(IntrinsicGeometryInterface &geometry,
                               const Eigen::MatrixXi &F,
                               const FaceData<Eigen::Matrix2d> &MrInv,
                               const FaceData<double> &kappa, // FaceData kappa (fixed)
                               double E,
                               double nu,
                               double h,
                               double w_s,
                               double w_b)
{
  ...
  // 1st fundamental form (stretching) — same as VD kappa version
  func.add_elements<6 + 6>(
      TinyAD::range(F.rows()),
      [&, alpha, beta, E, nu, h, w_s, w_b](auto &element) -> TINYAD_SCALAR_TYPE(element)
      {
  ...
  // 2nd fundamental form (bending) — kappa from FaceData directly
  geometry.requireVertexIndices();
  ...
  func.add_elements<3 * 6 + 3>(
      ...
      [&, alpha, beta, E, nu, h, w_s, w_b, kappa](auto &element) -> TINYAD_SCALAR_TYPE(element)
      {
        ...
        // Per-face kappa: direct access, NO averaging
        double kap = kappa[f];
