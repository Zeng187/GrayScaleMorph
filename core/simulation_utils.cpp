    ...
    const FaceData<Eigen::Matrix2d>& MrInv)
{
  ...
  trips.reserve(mesh.nFaces() * 6);

  for(Face f : mesh.faces())
  {
    // Recover flat reference edge vectors: Mr = MrInv^{-1}
    // Mr.col(0) = e01 = p1 - p0,  Mr.col(1) = e02 = p2 - p0
    Eigen::Matrix2d Mr = MrInv[f].inverse();
    Eigen::Vector2d e0 = Mr.col(0);
    Eigen::Vector2d e1 = Mr.col(1);

    // 2 * area = |e0 x e1| (scalar cross product in 2D)
    double cross = e0(0) * e1(1) - e0(1) * e1(0);
    double area2 = std::abs(cross);
    if(area2 < 1e-16) continue;

    // Vertex indices
    Halfedge he = f.halfedge();
    int v0 = static_cast<int>(geometry.vertexIndices[he.vertex()]);
    int v1 = static_cast<int>(geometry.vertexIndices[he.next().vertex()]);
    int v2 = static_cast<int>(geometry.vertexIndices[he.next().next().vertex()]);

    // Half-cotan weights for each edge:
    // Edge (v1,v2) opposite v0: w = cot(angle_v0) / 2 = (e0·e1) / (2*area2)
    double w12 = e0.dot(e1) / (2.0 * area2);
    // Edge (v0,v2) opposite v1: w = cot(angle_v1) / 2 = (|e0|^2 - e0·e1) / (2*area2)
    double w02 = (e0.squaredNorm() - e0.dot(e1)) / (2.0 * area2);
    // Edge (v0,v1) opposite v2: w = cot(angle_v2) / 2 = (|e1|^2 - e0·e1) / (2*area2)
    double w01 = (e1.squaredNorm() - e0.dot(e1)) / (2.0 * area2);

    // Assemble into Laplacian: L(i,j) -= w, L(i,i) += w
